...
- 2026-07-13: The first Windows hosted run reached the runner but failed in host-only unit-test prerequisites: Java was not on PATH, standalone `-pthread` compilation is not portable on Windows, and fake archive tests assumed POSIX separators. These tests now skip unavailable host-only checks or normalize Windows paths; the real Windows CastTuning validation remains the GN-built `cast_tuning_native_tests.exe` step.
